...
for x in range(0, 10):
    fold_folder = "./data/cholesterol/autopytorch/fold" + str(x + 1)
    ...
    autonet = AutoNetRegression(  # "tiny_cs",  # config preset
        log_level="info", budget_type="time", max_runtime=300, min_budget=1, max_budget=100
    )

    start = datetime.now().strftime("%H:%M:%S")
    perf = autonet.fit(
        X_train=X_train, Y_train=y_train, validation_split=0.25, early_stopping_patience=3
    )
