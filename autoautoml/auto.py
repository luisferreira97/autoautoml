...
class Autoautoml():
    ...
    def run_example(self, train_path, test_path, target):
        ...
        regressor = ak.StructuredDataRegressor(
            max_trials=10, loss="mean_absolute_error")
        ...
        label_column = target
        predictor = task.fit(
            train_data=train_data, label=label_column, eval_metric="mean_absolute_error")
        ...
        metrics["auto-gluon"] = predictor.evaluate_predictions(
            y_true=y_test, y_pred=y_pred, auxiliary_metrics=True)["mean_absolute_error"]
        ...
        tpot = TPOTRegressor(generations=5, population_size=50, verbosity=2,
                             random_state=42, scoring='neg_mean_absolute_error', cv=5)
        ...
        print("THE BEST AUTOML TOOL IS " + str(best_model)
              + ", WITH A MAE OF " + str(best_metric) + " ACHIEVED BY THE BEST MODEL.")
