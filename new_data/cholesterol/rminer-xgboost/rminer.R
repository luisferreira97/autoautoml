...
for (x in 0:9) {
    fold_folder = paste("/home/lferreira/autoautoml/new_data/cholesterol/rminer-xgboost/fold", as.character(x+1), sep="")
    ...
    M=fit(chol~.,data=train_df,model="xgboost",search=search,fdebug=TRUE)
    ...
    results = paste('{"time":',
                    ...
                    M@error,
                    ',"test_metric":',
                    round(mmetric(test_df$chol,P,metric=metric),2),
                    '}',
                    sep = "")
