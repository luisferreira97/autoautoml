...
for (x in 0:9) {
    fold_folder = paste("/home/lferreira/autoautoml/new_data/cholesterol/rminer/fold", as.character(x+1), sep="")
    ...
    M=fit(chol~.,data=train_df,model="auto",search=search,fdebug=TRUE)

    P=predict(M,test_df)

    test_metric = round(mmetric(test_df$chol,P,metric=metric),2)
    best_model = M@model
