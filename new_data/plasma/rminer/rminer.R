...
for (x in 0:9) {
    fold_folder = paste("/home/lferreira/autoautoml/new_data/plasma/rminer/fold", as.character(x+1), sep="")
    ...
    M=fit(RETPLASMA~.,data=train_df,model="auto",search=search,fdebug=TRUE)

    P=predict(M,test_df)

    test_metric = round(mmetric(test_df$RETPLASMA,P,metric=metric),2)
    best_model = M@model
