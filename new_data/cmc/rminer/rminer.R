...
data_path = "/home/lferreira/autoautoml/new_data/cmc/cmc"
label_column = 'Contraceptive_method_used'
metric="macroF1"
task = "class"
...
for (x in 0:9) {
    fold_folder = paste("/home/lferreira/autoautoml/new_data/cmc/rminer/fold", as.character(x+1), sep="")
    folds = list(fold1, fold2, fold3, fold4, fold5, fold6, fold7, fold8, fold9, fold10)
    test_df = folds[[(x+1)]]
    folds = folds[-(x+1)]
    train_df <- do.call(rbind, folds)

    train_df$Contraceptive_method_used = as.factor(train_df$Contraceptive_method_used)
    test_df$Contraceptive_method_used = as.factor(test_df$Contraceptive_method_used)
    ...
    M=fit(Contraceptive_method_used~.,data=train_df,model="auto",search=search,fdebug=TRUE)

    P=predict(M,test_df)

    test_metric = round(mmetric(test_df$Contraceptive_method_used,P,metric=metric),2)
    best_model = M@model
