...
for (x in 0:9) {
    fold_folder = paste("/home/lferreira/autoautoml/new_data/cloud/rminer-xgboost/fold", as.character(x+1), sep="")
    ...
    M=fit(TE~.,data=train_df,model="xgboost",search=search,fdebug=TRUE)
    ...
    results = paste('{"time":',
                    ...
                    M@error,
                    ',"test_metric":',
                    round(mmetric(test_df$TE,P,metric=metric),2),
                    '}',
                    sep = "")
