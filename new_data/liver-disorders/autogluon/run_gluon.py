...
data_path = "/home/lferreira/autoautoml/new_data/liver-disorders/liver-disorders"
...
for x in range(0, 10):
    fold_folder = "/home/lferreira/autoautoml/new_data/liver-disorders/autogluon/fold" + str(x+1)
    folds = [fold1, fold2, fold3, fold4, fold5, fold6, fold7, fold8, fold9, fold10]
    test_df = folds[x]
    del folds[x]
    ...
    predictor = task.fit(train_data=train_data, label=label_column, eval_metric="mean_absolute_error", num_bagging_folds=5, output_directory = fold_folder)
    end = datetime.now().strftime("%H:%M:%S")
    predictor.leaderboard().to_csv(fold_folder + "/leaderboard.csv")
    test_data = task.Dataset(test_df)
    y_test = test_data[label_column]
    y_pred = predictor.predict(test_data)
    perf = predictor.evaluate_predictions(y_true=y_test.to_numpy(), y_pred=y_pred, auxiliary_metrics=True)
    perf = dict(perf)
    perf["start"] = start
    perf["end"] = end
